...
**修复内容：**
1. **重新禁用被 reenable cron 误启用的 3 个失效源** — `mfuu/v2ray`（404）、`Epodonios/v2ray-configs-Trojan`（ReadError）、`awesome-vpn`（ReadError）再次被 reenable cron 重新启用。已重新禁用。

**验证结果：**
- 10 核心 API 回归测试全部 200
- 禁用源：mfuu/Epodonios/awesome-vpn（id=122/123/119）
- 节点池：总 13811 / active 132 / inactive 13679
- Python 语法检查通过
